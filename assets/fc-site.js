...
window.HeaderView = (function() {
  ...
  HeaderView.prototype.slideShowHeader = function($slideShow) {
    ...
    if ($slideShow.length && ($slideShow.attr('data-full-window-slideshow') != null) && $('body').hasClass('template-index') ) {
...
window.SlideshowView = (function(superClass) {
  ...
  SlideshowView.prototype.setSlideHeight = function(settings) {
    var $slide, $slideImage, $slideText, imageHeight, maxHeight, minHeight, slideHeight, slidePadding, textHeight, textWidth, windowHeight, windowWidth;
...
window.AlternateView = (function(superClass) {
  extend(AlternateView, superClass);

  function AlternateView() {
    return AlternateView.__super__.constructor.apply(this, arguments);
  }

  AlternateView.prototype.initialize = function() {
    this.$homeOrderNow = this.$('.home-order-now');
    this.$homeTestimonials = this.$('.home-testimonials');
    return this.sectionBinding();
  };

  AlternateView.prototype.render = function() {
    if (this.$homeOrderNow.length) {
      return new ProductView({
        el: this.$homeOrderNow
      });
  ...
  AlternateView.prototype.test = function() {
    console.log("ciaociao");
  }

  AlternateView.prototype.sectionBinding = function() {
    this.sections = new ThemeEditor();
    this.sections.register('slideshow', this.slideShow(this.sections));
    this.sections.register('full-width-feature', this.fullWidthFeature(this.sections));
    this.sections.register('alternating-content', this.alternatingContent(this.sections));
  ...
  AlternateView.prototype.slideShow = function(sections) {
    return {
      instances: {},
      el: '[data-section-type="slideshow"]',
      init: function(instance) {
  ...
  AlternateView.prototype.fullWidthFeature = function(sections) {
    return {
      instances: {},
      init: function(instance) {
        var $video;
  ...
  AlternateView.prototype.alternatingContent = function(sections) {
    return {
      instances: {},
      init: function(instance) {
        var $video;
  ...
  AlternateView.prototype.imagesWithText = function(sections) {
    return {
      instances: {},
      init: function(instance) {
        return this.instances[instance.sectionId] = new ImagesWithTextView({
  ...
  AlternateView.prototype.testimonials = function(sections) {
    return {
      instances: {},
      init: function(instance) {
        return this.instances[instance.sectionId] = new TestimonialView({
...
window.ThemeView = (function(superClass) {
  ...
  ThemeView.prototype.initialize = function() {
    var body, ref;
    body = $(document.body);
    this.isHowItWorks = body.find('.fc-how-it-works').length > 0;
    this.triggerJs = body.find('.fc-js-trigger').length > 0;
    this.triggerProductView = body.find('.fc-trigger-product-view').length > 0;
    this.isContactNew = body.find('.fc-contact-new').length > 0;
    
    
    
    this.isMediaKit = body.find('.fc-media-kit').length > 0;
  ...
  ThemeView.prototype.render = function() {
    ...
    if (this.triggerJs || this.isHowItWorks || this.isMediaKit || this.isCollection || this.isContactNew ) {
      this.alternateView = new AlternateView({
        el: this.$el
      });
      this.alternateView.render();
    }
    if (this.isHome) {
    ...
    if (this.isProduct || this.triggerProductView) {
      new ProductView({
        el: this.$el
      });
    }
...
function fc_slideshow_change(custom_class, dir) {
  ...
  var cs = $(s.find(".fc-slide.active")[0]);
  var csid = parseInt(cs.attr("data-slide"));
  //console.log("cs: " + cs.length + " - csid: " + csid);
  //console.log("fc_slideshow_change: " + custom_class + " - in dir " + dir + " - found: " + s.length + " container - " + s.find(".fc-slide[data-slide="+csid+"]").length + " slide");
  var nsid = csid + dir;
  if (nsid <= 0) {
    nsid = n;
  } else if (nsid > n || nsid == 'NaN') {
    nsid = 1;
  }
  console.log("Next slide: " + nsid);
  cs.removeClass("active");
  s.find(".fc-slide[data-slide="+nsid+"]").addClass("active");
}